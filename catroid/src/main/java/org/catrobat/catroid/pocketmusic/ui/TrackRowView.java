...
public class TrackRowView extends TableRow {
	...
	private void refreshNoteViews() {
		...
		if (tactPosition % 2 == 0) {
			whiteKeyColor = ContextCompat.getColor(getContext(), R.color.pocketmusic_white_piano_key);
			blackKeyColor = ContextCompat.getColor(getContext(), R.color.pocketmusic_black_piano_key);
		} else {
			whiteKeyColor = ContextCompat.getColor(getContext(), R.color.pocketmusic_white_piano_key_dark);
			blackKeyColor = ContextCompat.getColor(getContext(), R.color.pocketmusic_black_piano_key_dark);
		}
	...
	private List<GridRowPosition> getGridRowsForCurrentTact() {
		return gridRow.gridRowsForTact(tactPosition);
	}
